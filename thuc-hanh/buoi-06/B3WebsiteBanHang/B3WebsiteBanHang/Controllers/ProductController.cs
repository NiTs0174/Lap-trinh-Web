...
//------------------------------------SEARCH-----------------------------------------------
        ...
        {
            return _context.Products.Where(p => p.Name.StartsWith(query)).Select(p => p.Name).ToList();
        }
        //---------------------------------
        public async Task<IActionResult> SearchProducts(string query, int pageNumber = 1)
        {
            IQueryable<Product> productsQuery = _context.Products.Include(p => p.Category).Where(p => p.Name.Contains(query));
